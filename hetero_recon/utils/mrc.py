...
MODE_FOR_DTYPE = {
    vv: kk
    for kk, vv in DTYPE_FOR_MODE.items()
}
...
class MRCHeader:
    ...
    @classmethod
    def make_default_header(cls, data, is_vol=True, a_pix=1., xorg=0., yorg=0., zorg=0.):
        nz, ny, nx = data.shape
    ...
    def update_apix(self, a_pix):
        self.fields['xlen'] = self.fields['nx'] * a_pix
        self.fields['ylen'] = self.fields['ny'] * a_pix
        self.fields['zlen'] = self.fields['nz'] * a_pix
...
def write(fname, array, header=None, a_pix=1., xorg=0., yorg=0., zorg=0., is_vol=None):
